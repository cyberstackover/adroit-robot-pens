...
unsigned int LaguTick=0;
interrupt [TIM2_COMP] void timer2_comp_isr(void)
{       //TCCR2=0x0;      // stop timer
    PIDTick++;
    if(LaguOn)
    {   SysTick++;
        if(SysTick>=(Tempo[LaguTick]*12))
        {   SysTick = 0;
            LaguTick++;
...
{   PwmM2H = Ka>>8; PwmM2L = (unsigned char) Ka;
    PwmM1H = Ki>>8; PwmM1L = (unsigned char) Ki;
    M2_CW;    M1_CCW;   
}
#pragma used-
#pragma used+
...
{ int PWMKi, PWMKa, U;
  static signed char lError=0;
  static signed int  iError=0;
  unsigned char PIDTime; 
  PIDTime = PIDTick;
  PIDTick = 0;
  U = (KD * (Error-lError))/PIDTime ;
  iError += (Error + lError)/2 * (signed int)PIDTime ; 
  U+= KI * iError;
  lError = Error;
  U+= KP * Error;
  U/=Pembagi;
...
float  iError=0;
void PID(unsigned int Speed, signed char Error)
{ int PWMKi, PWMKa;
  static char   lError=0;
  float PIDTime, U; 
  PIDTime = (float)PIDTick/1000;
  PIDTick = 0;
  // Menghitung Nilai Kendali Derivatif
  U = KD * (float)(Error-lError)/PIDTime ;
  // Menghitung Nilai Integral Error
  iError += ((float)Error + (float)lError)/2.0 * PIDTime ; 
  // Membatasi Nilai Integral Error
  if((signed int) iError > MaxIntegral) iError = MaxIntegral;
  ...
  if (PWMKa<MaxSpeed) 
    {    PwmM2H = (PWMKa & 0xFF00) >> 8; PwmM2L = (unsigned char) PWMKa;}
  else
    {    PwmM2H = MaxSpeed>>8; PwmM2L = (unsigned char) MaxSpeed;}
  // Menghitung Kecepatan dan arah putaran Motor Roda Kiri
  if(PWMKi<0)   {PWMKi = -PWMKi; M1_CW;}
  else          {M1_CCW;}
  if (PWMKi<MaxSpeed) 
    {    PwmM1H = (PWMKi & 0xFF00) >>8; PwmM1L = (unsigned char) PWMKi;}
  else
    {    PwmM1H = MaxSpeed>>8; PwmM1L = (unsigned char) MaxSpeed;}
  LCD_GotoXY(0,0); LCD_Angka3(PIDTime*1000);
  //LCD_Data(' '); LCD_Angka3(PWMKa);
