...
//  --------------  DEKLARASI VARIABEL GLOBAL --------------------------
    ...
    signed char      dMotor1, dMotor2;
        ...
        #endif
        volatile short int Ax=0, Ay=0, Az=0, Gx=0, Gy=0, Gz=0;
        volatile float GxOffset, GyOffset, GzOffset;         // 1000 x nilai sali
        volatile float Roll, Pitch, Yaw;                     // 1000 x nilai asli
    #endif
...
#define Rad2Deg     (float) 57,295779513    //082320876798154814105  
#define Deg2Rad     (float) 0,0174532925    //1994329576923690768489 


void main(void)
{   unsigned char r,g,b,i, lcd[16]; 
    SystemInit(); 
    Stop();
    I2C_Init();
	MPU6050_Init();   
    ...
    {   //float Axg, Ayg, Azg, Gxrs, Gyrs, Gzrs;
        ...
        //UpdateMahonyFilter2();
        //UpdateRollPitchYaw();
        //Roll+=((float)Gx-GxOffset)*0.000609756097561f;
        //Pitch+=((float)Gy-GyOffset)*0.000609756097561f;
        //Yaw+=((float)Gz-GzOffset)*0.000609756097561f;
        
        Roll += ((float)Gx-GxOffset) * 0.0006211180124223602484472049689441;
        Pitch += ((float)Gy-GyOffset) * 0.0006211180124223602484472049689441;
        Yaw += ((float)Gz-GzOffset) * 0.0006211180124223602484472049689441;
        
         
        if(SysTick>i)    r = SysTick - i;
        else             r = 0xFF - i + SysTick + 1;
        LCD_Hapus();
        
        //Gx=((float)Gx-(float)GxOffset);// *0.1f;//  (float)(0.00106422515365508);
        //Gy=((float)Gy-(float)GyOffset);// *0.1f;//(float)(0.00106422515365508);
        //Gz=((float)Gz-(float)GzOffset);// *0.1f;//(float)(0.00106422515365508);
        
        
        /*sprintf(lcd,"%.2f",Gx);
        LCD_GotoXY(0,0);    LCD_Text(lcd);
        sprintf(lcd,"%.2f",Gy);
        LCD_GotoXY(8,0);    LCD_Text(lcd);
        sprintf(lcd,"%.2f",Gz);
        LCD_GotoXY(0,1);    LCD_Text(lcd);
        */
        
        if(MODE1)
        {
            LCD_GotoXY(0,0);    LCD_Angka4((int)(Roll));
            LCD_GotoXY(8,0);    LCD_Angka4((int)(Pitch));
