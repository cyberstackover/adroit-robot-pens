...
void LCD_TulisKiri(uint8_t Baris, uint8_t flash *text)    // menuliskan string dari arah kiri dgn delay pada (0=baris 1, 1=Baris 2)
{   uint8_t i;
    LCD_GotoXY(0,Baris);
    for(i=0;i<16;i++)
...
void LCD_TulisKanan(uint8_t Baris, uint8_t flash *text)    // menuliskan string dari arah kanan dgn delay 
{   int8_t i,j;
    j = 0x80 + 0x40*Baris;
    for(i=15;i>-1;i--)
...
void LCD_TulisTengah(uint8_t Baris, uint8_t flash *text)    // menuliskan string dari arah kanan dgn delay 
{   int8_t i,j;
    j = 0x80 + 0x40*Baris;
    for(i=7;i>-1;i--)
    {   LCD_Perintah (j+i);     LCD_Data(*(text+i));    delay_ms(DelayLCD);
...
void LCD_TulisPinggir(uint8_t Baris, uint8_t flash *text)    // menuliskan string dari arah kanan dgn delay 
{   int8_t i,j;
    j = 0x80 + 0x40*Baris;
    for(i=0;i<8;i++)
    {   LCD_Perintah (j+i);     LCD_Data(*(text+i));    delay_ms(DelayLCD);
